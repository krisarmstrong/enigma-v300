...
const std::vector<ProductOptions> PRODUCT_OPTIONS = {
    ...
    {"7001", {{"000", "802.1x"}, {"002", "Reports"}, {"003", "LAN"}}},
    {"2186",
    ...
    {"3001", {{"003", "Personalization"}, {"004", "VoIP"}, {"005", "NetSecure"}, {"008", "Dicom"}}},
};
...
const std::vector<int> ENIGMA2_E_ROTOR_26 = {16, 8, 25, 5, 23, 21, 18, 17, 2,  1,  7, 24, 15,
                                             11, 9, 6,  3, 0,  19, 12, 22, 14, 10, 4, 20, 13};
const std::vector<int> ENIGMA2_D_ROTOR_10 = {6, 2, 7, 5, 1, 0, 9, 4, 3, 8};
const std::vector<int> ENIGMA2_D_ROTOR_26 = {17, 9,  8,  16, 23, 3, 15, 10, 1, 14, 22, 13, 19,
...
void enigma_c_encrypt(const std::string& input_key, std::string& output_key, size_t len) {
    ...
    for (size_t index = 0; index < len; ++index) {
        ...
        output_key.at(index) =
            (temp < 10) ? static_cast<char>(temp + '0') : static_cast<char>(temp - 10 + 'a');
    }
}
...
bool enigma_c_check_option_key(int option, const std::string& key,
    ...
    if (decoded_input.substr(0, SERIAL_NUMBER_SIZE_ENIGMAC) != serial_number)
        return false;
...
void enigma2_c_encrypt(const std::string& input_key, std::string& output_key) {
    ...
    for (size_t i = 0; i < KEY_LENGTH; ++i) {
        ...
        if (std::isdigit(static_cast<unsigned char>(output_key.at(i)))) {
            ...
            output_key.at(i) = static_cast<char>(
                ENIGMA2_E_ROTOR_10.at((temp_sum + MAX_CHECK_SUM - running_sum) % 10) + '0');
        } else {
            // Narrowing conversion is intentional: result is guaranteed to be in range 0-25
            output_key.at(i) = static_cast<char>(
                'A' + ENIGMA2_E_ROTOR_26.at((temp_sum + MAX_CHECK_SUM - running_sum) % 26));
        }
...
bool product_code_menu(std::string& product_code, std::string& option_code) {
    ...
    for (size_t i = 0; i < PRODUCT_TABLE.size(); ++i) {
        std::cout << (i + 1) << ". " << PRODUCT_TABLE.at(i).code << " - "
                  << PRODUCT_TABLE.at(i).name << "\n";
    }
    ...
    for (size_t i = 0; i < options->options.size(); ++i) {
        std::cout << (i + 1) << ". " << options->options.at(i).code << " - "
                  << options->options.at(i).desc << "\n";
    }
...
void calculate_nettool_option_key(std::string& serial_number, int option_number) {
    if (serial_number.empty()) {
        while (true) {
            ...
            if (serial_number.length() == SERIAL_NUMBER_SIZE_ENIGMAC &&
    ...
    if (option_number < 0) {
        ...
        std::getline(std::cin, input);
        option_number =
            (input.length() > 0 && std::isdigit(static_cast<unsigned char>(input.at(0))))
                ? (input.at(0) - '0')
                : 0;
    }
    if (option_number < 0 || option_number > 9)
        option_number = 0;
...
void check_nettool_option_key(std::string& option_key) {
    ...
    if (option_key.empty()) {
        while (true) {
            ...
            if (option_key.length() == 12 &&
                std::all_of(option_key.begin(), option_key.end(),
...
void calculate_enigma2_option_key(std::string& serial_number, int option_number, int product_code,
                                  bool assume_escope) {
    ...
    if (serial_number.empty()) {
        while (true) {
            ...
            std::getline(std::cin, serial_number);
            if (serial_number.length() == SERIAL_NUMBER_SIZE_ENIGMA2 &&
                std::all_of(serial_number.begin(), serial_number.end(), ::isdigit))
                break;
...
void check_enigma2_option_key(std::string& option_key) {
    if (option_key.empty()) {
        while (true) {
            ...
            std::getline(std::cin, option_key);
            if (option_key.length() == 16 &&
                std::all_of(option_key.begin(), option_key.end(), [](char c) {
                    return std::isalnum(static_cast<unsigned char>(c)) &&
                           (std::isdigit(static_cast<unsigned char>(c)) || (c >= 'A' && c <= 'Z'));
                }))
                break;
...
int main(int argc, char* argv[]) {
    ...
    if (argc > 1) {
        ...
        if (argc > 2) {
            ...
            if (argc > 3)
                option_number = std::stoi(argv[3]);
            if (argc > 4)
                product_code = std::stoi(argv[4]);
        }
    }
