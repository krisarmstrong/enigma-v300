...
#
# NOTE: C++ CLI supports: -n, -x, -e, -l, -d, -?
# Missing (vs C impl): --version, --help, --list-products, --list-options
...
# Test: Help output
echo "--- Help Tests ---"
HELP_OUTPUT=$("$ENIGMA" -? 2>&1 || true)
check_output "Help shows encrypt option" "-e" "$HELP_OUTPUT"
check_output "Help shows decrypt option" "-d" "$HELP_OUTPUT"
check_output "Help shows NetTool option" "-n" "$HELP_OUTPUT"
check_output "Help shows LinkRunner option" "-l" "$HELP_OUTPUT"
...
# Note: -l shortcut requires 3 args or enters interactive mode
# Skipped to avoid hanging tests
